...
public class FaceFXControllerScriptGUI : Editor
{
    ...
    public struct BoneTransform
    {
        ...
        public BoneTransform(string aValue)
        {
            ...
            {
                Debug.Log("Error in XML.  A reference bone has only this many values:" + Values.Length);
                Debug.Log(aValue);
            }
            else
            {
    ...
    public void ImportXMLAnimations(string xmltext)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < animNodeList.Count; ++i)
            {
                ...
                if (null != _target.GetComponent<Animation>()[animGroupName + "_" + animName])
                {
                    Debug.Log("Replacing existing FaceFX animation: " + animGroupName + "_" + animName);
                }
                controllerAnimHelper.animclip.legacy = true;
                _target.GetComponent<Animation>().AddClip(controllerAnimHelper.animclip, animGroupName + "_" + animName);
                numAnimsImported++;
            }
            ...
            for (int i = 0; i < animEventTakeList.Count; ++i)
            {
                ...
                if (null != _target.GetComponent<Animation>()[eventTrackAnimName])
                {
                    if (numEvents > 0)
                    {
                    ...
                    _target.GetComponent<Animation>().RemoveClip(eventTrackAnimName);
                }
                if (numEvents > 0)
                {
                    _target.GetComponent<Animation>().AddClip(new AnimationClip(), eventTrackAnimName);
    ...
    public void ImportXML(string xmltext)
    {
        ...
        if (faceGraphNodes.Count > 0)
        {
                ...
                for (j = 0; j < bonesNode.ChildNodes.Count; ++j)
                {
                    ...
                    else
                    {
                        ...
                        if (boneObject)
                        {
                            ...
                            bonePoseHelper.animclip.legacy = true;
                            _target.GetComponent<Animation>().AddClip(bonePoseHelper.animclip, "facefx " + bonePoseName);
                        }
                    }
                }
            }
            ...
            loopAnim.animclip.legacy = true;
            _target.GetComponent<Animation>().AddClip(loopAnim.animclip, "facefx_loop_anim");
