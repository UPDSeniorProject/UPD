...
//------------------------------------------------------------------------	
	...
	public class Matrix
	{
		...
		{return new Matrix(Inverse(this.in_Mat));}
		#endregion
		...
		{return new Matrix(Transpose(this.in_Mat));}																			
		#endregion
		...
		public void SVD(out Matrix S, out Matrix U, out Matrix V)
		{
			double [,] s, u, v;
			SVD(this.in_Mat, out s, out u, out v);
			S = new Matrix(s);
			U = new Matrix(u);
			V = new Matrix(v);
		}
		...
		public void LU( out Matrix L, out Matrix U, out Matrix P)
		{
			double [,] l, u, p;
			LU(this.in_Mat, out l, out u, out p);
			L = new Matrix(l);
			U = new Matrix(u);
			P = new Matrix(p);
		}
		...
		public Matrix SolveLinear( Matrix MatB)
		{return new Matrix(Matrix.SolveLinear(this.in_Mat, MatB.in_Mat));}
		#endregion
		...
		public Matrix PINV()
		{return new Matrix(PINV(this.in_Mat));}
		#endregion
		...
		public Matrix CrossProduct(Matrix V2)
		{return (new Matrix((CrossProduct(this.in_Mat,V2.in_Mat))));}
		#endregion
		...
		/// <returns>Dot product of V1 and V2</returns>
		public double DotProduct(Matrix V2)
		{return (DotProduct(this.in_Mat, V2.in_Mat));}
		#endregion
		...
		{return (VectorMagnitude(this.in_Mat));}					
		#endregion
		...
		/// <returns>If two matrices are equal or not</returns>
		public bool IsEqual (Matrix Mat2)
		{return IsEqual(this.in_Mat, Mat2.in_Mat);}
		...
		/// <summary>
		/// Serves as a hash function for a particular type. (This override prevents a couple warnings, even if we are not changing the hashcode computation)
		/// </summary>
		...
		// For vectors only, doncha be callin this on a matrix!
		public double Norm2() {
			int Rows, Cols;
			Find_R_C(this.in_Mat,out Rows, out Cols);
			...
			if (Rows == 1) {
				for (int i = 0; i <= Cols; i ++) {
					returnval += this.in_Mat[0,i] * this.in_Mat[0,i];
				}
			} else {
				for (int i = 0; i <= Rows; i ++) {
					returnval += this.in_Mat[i,0] * this.in_Mat[i,0];
				}
			}
		...
		public static double[,] ScalarSubtract(double Value, double[,] Mat)
		{
			int i, j, Rows, Cols;
			double[,] sol;
			...
			for (i = 0; i<=Rows;i++)
				for (j = 0; j<=Cols;j++)
					sol[i, j] = Value - Mat[i, j];

			return sol;
		}
	
	
		public static double[,] ScalarSubtract(double[,] Mat, double Value)
		{
			int i, j, Rows, Cols;
			double[,] sol;
		...
		public static Matrix operator-(Matrix mat, double scalar) {
			return new Matrix(ScalarSubtract(mat.in_Mat, scalar));
		}
	
		public static Matrix operator-(double scalar, Matrix mat) {
			return new Matrix(ScalarSubtract(scalar, mat.in_Mat));
		}
	
		public static double[,] ScalarAdd(double Value, double[,] Mat)
		{
			int i, j, Rows, Cols;
			double[,] sol;
		...
		public static double[,] ScalarAdd(double[,] Mat, double Value)
		{
			int i, j, Rows, Cols;
			double[,] sol;
		...
		public static Matrix operator+(Matrix mat, double scalar) {
			return new Matrix(ScalarAdd(mat.in_Mat, scalar));
		}
	
		public static Matrix operator+(double scalar, Matrix mat) {
			return new Matrix(ScalarAdd(scalar, mat.in_Mat));
		}
	
		public Matrix Col(int col) {
			int Rows, Cols;
			
			try  {Find_R_C(this.in_Mat, out Rows, out Cols);}
			catch{throw new MatrixNullException();}
		
			if (col < 0 || col > Cols + 1)
				throw new MatrixDimensionException();
		
			Matrix returnval = new Matrix(Rows + 1, 1);
			
			for (int i = 0; i < Rows + 1; i ++) {
				returnval[i,0] = this.in_Mat[i,col];
			}
			
			return returnval;
		}
	
		public void SetColumn(int col, Matrix newValues) {
			int Rows1, Cols1, Rows2, Cols2;
			
			try  {Find_R_C(this.in_Mat, out Rows1, out Cols1);}
			catch{throw new MatrixNullException();}
		
			try  {Find_R_C(newValues.in_Mat, out Rows2, out Cols2);}
			catch{throw new MatrixNullException();}
		
			if (col < 0 || col > Cols1 + 1 || Rows1 != Rows2)
				throw new MatrixDimensionException();
		
			
			for (int i = 0; i < Rows1 + 1; i ++) {
				this.in_Mat[i,col] = newValues[i,0];
			}
		...
		public bool AnyNaN() {
			...
			for (int i = 0; i < Rows + 1; i ++) {
				for (int j = 0; j < Cols + 1; j ++) {
					if (Double.IsNaN(this.in_Mat[i,j]))
						return true;
				}
			}
