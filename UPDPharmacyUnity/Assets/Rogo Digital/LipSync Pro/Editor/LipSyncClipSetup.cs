...
#pragma warning disable 618
    ...
    private const float version = 1.32f;
    private const string versionNumber = "Pro 1.32";
    ...
    void OnEnable () {
        ...
        if (keyboardShortcuts == null) SetDefaultShortcuts();
    ...
    public override void OnModalGUI () {
        ...
        if (GUILayout.Button("Edit", EditorStyles.toolbarDropDown, GUILayout.Width(60))) {
            ...
            editMenu.AddSeparator("");
            if (markerTab == 0 || markerTab == 1) editMenu.AddItem(new GUIContent("Default Marker Settings"), false, DefaultMarkerSettings);
        ...
        if (Event.current.type == EventType.Repaint) previewRect = GUILayoutUtility.GetLastRect(); // Only set previewRect dimensions on Repaint, GetLastRect is incorrect on Layout.
...
#if UNITY_5_3_6 || UNITY_5_4_OR_NEWER
        if (clip != null) AudioUtility.DrawWaveForm(clip, 0, previewRect, viewportStart, viewportSeconds);
#else
            ...
            if (dragging == false && highlightedMarker > -1 && focusedWindow == this) {
                ...
                if (Event.current.type == EventType.MouseDrag) {
                    ...
                    if (selection.Count > 1) {
                        ...
                        if (selection.Count > 1) {
                            markerMenu.AddItem(new GUIContent("Delete"), false, DeleteSelectedPhonemes);
                        } else {
            ...
            if (dragging == false && highlightedMarker > -1 && focusedWindow == this) {
                ...
                if (Event.current.type == EventType.MouseDrag) {
                    ...
                    if (selection.Count > 1) {
                        ...
                        if (!emotionData[highlightedMarker].isMixer) markerMenu.AddItem(new GUIContent("Change/Replace with New Mixer"), false, ChangeEmotionMixer, emotionData[highlightedMarker]);
                        ...
                        if (emotionData[highlightedMarker].isMixer) markerMenu.AddItem(new GUIContent("Edit Mixer"), false, OpenEmotionMixer, emotionData[highlightedMarker]);
                    }
                    markerMenu.ShowAsContext();
        ...
        if (phonemesNode != null) {
            ...
            for (int p = 0; p < phonemeNodes.Count; p++) {
                ...
                if (node.LocalName == "marker") {
                    string phonemeName = node.Attributes["phoneme"].Value;
                    ...
                    if (found) {
                        ...
                        Debug.LogError("Loading XML Failed: Phoneme '" + phonemeName + "' does not exist in the current set.");
                        phonemeData.Clear();
                        return;
                    }
    ...
    public static LipSyncData SaveFile (LipSyncProject settings, string path, bool isXML, string transcript, float fileLength, PhonemeMarker[] phonemeData, EmotionMarker[] emotionData, GestureMarker[] gestureData, AudioClip clip = null) {
        if (isXML) {
            ...
            foreach (PhonemeMarker marker in phonemeData) {
                ...
                writer.WriteAttributeString("phoneme", settings.phonemeSet.phonemes[marker.phonemeNumber].name);
