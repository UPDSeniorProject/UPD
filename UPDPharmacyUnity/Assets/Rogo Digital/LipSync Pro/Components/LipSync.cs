...
#pragma warning disable 618
        ...
        public void Play (LipSyncData dataFile, float delay) {
            if (ready && enabled) {
                ...
                // Play audio
                if (audioClip && audioSource) audioSource.PlayDelayed(delay);
            }
        }
            ...
            if (IsPlaying && enabled) {
                ...
                if (useBones) {
                    for (int b = 0; b < boneCurves.Count; b++) {
                        ...
                        boneCurves[b].AddKey(0, startingPosition, startingRotation, 0, 0);
                        boneCurves[b].AddKey(1, finalPosition, finalRotation, 0, 0);
                    }
                }
                            ...
                            if (shape.HasBone(tempEmotionBones[index])) {
                                ...
                                if (marker.blendFromMarker) {
                                    ...
                                    if (prevShape.HasBone(tempEmotionBones[index])) {
                                        ...
                                        startRotation = Vector3.Lerp(startRotation, prevShape.bones[prevShape.IndexOfBone(tempEmotionBones[index])].endRotation, prevMarker.intensity);
                                    }
                                }
                                ...
                                if (marker.blendToMarker) {
                                    ...
                                    if (nextShape.HasBone(tempEmotionBones[index])) {
                                        ...
                                        endRotation = Vector3.Lerp(endRotation, nextShape.bones[nextShape.IndexOfBone(tempEmotionBones[index])].endRotation, nextMarker.intensity);
                                    }
                                }
                                ...
                                if (shape.bones[b].lockPosition && shape.bones[b].lockRotation) {
                                    ...
                                    for (int i = 0; i < variationPoints; i++) {
                                        float time = Mathf.Lerp(marker.startTime + marker.blendInTime, marker.endTime + marker.blendOutTime, (float)i / (float)(variationPoints - 1));
                                        tempEmotionBoneCurves[index].AddKey(time, Quaternion.Euler(Vector3.Lerp(shape.bones[b].neutralRotation, shape.bones[b].endRotation * boneRotMod[i], marker.intensity * intensityMod[i])), 0, 0);
                                    }
                                    ...
                                    for (int i = 0; i < variationPoints; i++) {
                                        float time = Mathf.Lerp(marker.startTime + marker.blendInTime, marker.endTime + marker.blendOutTime, (float)i / (float)(variationPoints - 1));
                                        tempEmotionBoneCurves[index].AddKey(time, Vector3.Lerp(shape.bones[b].neutralPosition, shape.bones[b].endPosition * bonePosMod[i], marker.intensity * intensityMod[i]), 0, 0);
                                    }
                                    ...
                                    for (int i = 0; i < variationPoints; i++) {
                                        float time = Mathf.Lerp(marker.startTime + marker.blendInTime, marker.endTime + marker.blendOutTime, (float)i / (float)(variationPoints - 1));
                                        tempEmotionBoneCurves[index].AddKey(time, Vector3.Lerp(shape.bones[b].neutralPosition, shape.bones[b].endPosition * bonePosMod[i], marker.intensity * intensityMod[i]), Quaternion.Euler(Vector3.Lerp(shape.bones[b].neutralRotation, shape.bones[b].endRotation * boneRotMod[i], marker.intensity * intensityMod[i])), 0, 0);
                                    }
                                ...
                                if (marker.blendToMarker) {
                                    ...
                                    if (nextShape.HasBone(tempEmotionBones[index])) {
                                        ...
                                        if (b.lockPosition && b.lockRotation) {
                                            ...
                                            tempEmotionBoneCurves[index].AddKey(marker.endTime, Quaternion.Euler(Vector3.Lerp(b.neutralRotation, b.endRotation, nextMarker.intensity)), 0, 0);
                                        } else if (b.lockRotation) {
                                            tempEmotionBoneCurves[index].AddKey(marker.endTime, Vector3.Lerp(b.neutralPosition, b.endPosition, nextMarker.intensity), 0, 0);
                                        } else {
                                            tempEmotionBoneCurves[index].AddKey(marker.endTime, Vector3.Lerp(b.neutralPosition, b.endPosition, nextMarker.intensity), Quaternion.Euler(Vector3.Lerp(b.neutralRotation, b.endRotation, nextMarker.intensity)), 0, 0);
                                        }
                                    }
                                }
                            }
                        ...
                        for (int index = 0; index < tempBoneCurves.Count; index++) {
                            if (shape.HasBone(bones[index])) {
                                ...
                                if (shape.bones[b].lockPosition && shape.bones[b].lockRotation) {
                                    ...
                                    tempBoneCurves[index].AddKey(marker.time, Quaternion.Euler(Vector3.Lerp(shape.bones[b].neutralRotation, shape.bones[b].endRotation * boneRotMod, marker.intensity * intensityMod)), 0, 0);
                                } else if (shape.bones[b].lockRotation) {
                                    tempBoneCurves[index].AddKey(marker.time, Vector3.Lerp(shape.bones[b].neutralPosition, shape.bones[b].endPosition * bonePosMod, marker.intensity * intensityMod), 0, 0);
                                } else {
                                    tempBoneCurves[index].AddKey(marker.time, Vector3.Lerp(shape.bones[b].neutralPosition, shape.bones[b].endPosition * bonePosMod, marker.intensity * intensityMod), Quaternion.Euler(Vector3.Lerp(shape.bones[b].neutralRotation, shape.bones[b].endRotation * boneRotMod, marker.intensity)), 0, 0);
                                }
        ...
        private void LoadXML (TextAsset xmlFile, AudioClip linkedClip) {
            ...
            if (float.Parse(version) < 1.3f) {
                Debug.LogError("Cannot load pre-1.3 XML file. Run the converter from Window/Rogo Digital/LipSync Pro/Update XML files.");
                return;
            }
                ...
                if (phonemesNode != null) {
                    ...
                    for (int p = 0; p < phonemeNodes.Count; p++) {
                        ...
                        if (node.LocalName == "marker") {
                            int phoneme = int.Parse(node.Attributes["phoneme"].Value);
